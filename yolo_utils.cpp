...
void GetMask2(const Mat& maskProposals, const Mat& mask_protos, Output& output, const MaskParams& maskParams) {
    int seg_channels = maskParams.segChannels;
    int net_width = maskParams.netWidth;
    int seg_width = maskParams.segWidth;
    int net_height = maskParams.netHeight;
...
void DrawPred(Mat& img, vector<Output> result, std::vector<std::string> classNames, vector<Scalar> color) {
    Mat mask = img.clone();
