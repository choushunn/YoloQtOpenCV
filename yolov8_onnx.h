...
public:
    ...
    */
    bool OnnxDetect(cv::Mat& srcImg, std::vector<Output>& output);
    /** \brief  detect,batch size= _batchSize
    * \param[in] srcImg:A batch of images.
    * \param[out] output:detection results of input images.
    */
    bool OnnxBatchDetect(std::vector<cv::Mat>& srcImg, std::vector<std::vector<Output>>& output);
...
private:
    ...
    const int _netWidth = 1280;  //ONNX input image width
    const int _netHeight = 1280; //ONNX input image height
